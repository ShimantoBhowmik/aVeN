...
async def main():
    ...
    try:
        ...
        if results["successful"] > 0:
            logger.info(
                f"Crawling completed successfully. {results['successful']} pages processed."
            )
            return 0
        else:
...
def run_crawler():
    ...
    try:
        ...
        if crawler_exit_code == 0 or pdf_success:
            ...
            print(
                f"All extracted content is in: {os.path.abspath(DEFAULT_OUTPUT_FOLDER)}"
            )
            sys.exit(0)
        else:
...
def run_pdf_processor():
    ...
    try:
        ...
        # Count PDF files
        pdf_files = [
            f for f in os.listdir(DEFAULT_PDF_FOLDER) if f.lower().endswith(".pdf")
        ]
        if not pdf_files:
        ...
        if results["successful"] > 0:
            logger.info(
                f"PDF processing completed successfully. {results['successful']} files processed."
            )
            return True
        else:
