...
class Crawler:
    ...
    def __init__(self, output_folder: str = DEFAULT_OUTPUT_FOLDER):
        ...
        self.logger.info(f"Crawler initialized with output folder: {self.output_folder}")
    
    def _ensure_output_folder(self) -> None:
        """Ensure the output folder exists"""
        try:
            os.makedirs(self.output_folder, exist_ok=True)
            self.file_logger.info(f"Output folder created/verified: {self.output_folder}")
        except Exception as e:
            self.file_logger.error(f"Failed to create output folder {self.output_folder}: {e}")
            raise
    ...
    async def _extract_content_from_page(self, page, url: str) -> ContentData:
        ...
        try:
            ...
            self.content_logger.info(f"Extracted {len(cleaned_content)} characters from {url}")
        ...
        for selector in MAIN_CONTENT_SELECTORS:
            try:
                ...
                if await element.count() > 0:
                    ...
                    self.content_logger.debug(f"Found content using selector: {selector}")
                    break
            except Exception as e:
    ...
    def _is_content_valid(self, content_data: ContentData) -> bool:
        ...
        if len(content_data.content) < MIN_CONTENT_LENGTH:
            self.content_logger.warning(f"Content too short for {content_data.url}: {len(content_data.content)} chars")
            return False
        
        # Check for JavaScript error messages
        if "javascript" in content_data.content.lower() and len(content_data.content) < 200:
            self.content_logger.warning(f"Detected JavaScript error message for {content_data.url}")
            return False
    ...
    async def crawl_urls(self, urls: List[str]) -> Dict[str, any]:
        ...
        # Log final results
        self.logger.info(f"Crawl completed - Success: {results['successful']}, Failed: {results['failed']}")
