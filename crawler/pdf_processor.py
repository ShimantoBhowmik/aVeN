...
try:
    ...
    print(f"Missing PDF processing libraries. Please install: pip install PyPDF2 pdfplumber pytesseract Pillow pdf2image")
    raise e

from constants import DEFAULT_PDF_FOLDER, DEFAULT_OUTPUT_FOLDER, MIN_CONTENT_LENGTH
...
class PDFContentData:
    ...
    def __init__(self, filename: str, title: str = "", content: str = "", 
                 method: str = "", pages: int = 0, status: str = ""):
...
class PDFProcessor:
    ...
    def __init__(self, pdf_folder: str = DEFAULT_PDF_FOLDER, 
    ...
    def _ensure_folders(self) -> None:
        ...
        try:
            ...
            self.file_logger.info(f"Folders verified: {self.pdf_folder}, {self.output_folder}")
    ...
    def _clean_text(self, text: str) -> str:
        ...
        self.content_logger.debug(f"Cleaned text from {len(text)} to {len(cleaned)} characters")
        return cleaned
    ...
    def _generate_safe_filename(self, pdf_filename: str) -> str:
        ...
        output_filename = f"PDF_{safe_name}_{file_hash}.txt"
        
        self.file_logger.debug(f"Generated filename '{output_filename}' for PDF: {pdf_filename}")
            ...
            with open(pdf_path, 'rb') as file:
                ...
                for page_num, page in enumerate(pdf_reader.pages):
                    try:
                        ...
                        self.content_logger.warning(f"Failed to extract page {page_num + 1}: {e}")
    ...
    def _extract_with_pdfplumber(self, pdf_path: str) -> PDFContentData:
        ...
        try:
            ...
            self.content_logger.error(f"pdfplumber extraction failed for {pdf_path}: {e}")
            return PDFContentData(
                filename=os.path.basename(pdf_path),
                method="pdfplumber",
                status=f"error: {str(e)}"
        ...
        try:
            ...
            for page_num, image in enumerate(images):
                try:
                    ...
                    self.content_logger.warning(f"OCR failed for page {page_num + 1}: {e}")
    ...
    def _extract_pdf_content(self, pdf_path: str) -> PDFContentData:
        ...
        """
        self.content_logger.info(f"Extracting content from: {os.path.basename(pdf_path)}")
        ...
        if result.status == "success" and len(result.content) > MIN_CONTENT_LENGTH:
            self.content_logger.info(f"Successfully extracted with PyPDF2: {len(result.content)} chars")
            return result
        ...
        if result.status == "success" and len(result.content) > MIN_CONTENT_LENGTH:
            self.content_logger.info(f"Successfully extracted with pdfplumber: {len(result.content)} chars")
            return result
        ...
        if result.status == "success":
            self.content_logger.info(f"Successfully extracted with OCR: {len(result.content)} chars")
        else:
    ...
    def _save_content_to_file(self, content_data: PDFContentData) -> Optional[str]:
        ...
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(f"Source PDF: {content_data.filename}\n")
                f.write("=" * 50 + "\n\n")
    ...
    def _find_pdf_files(self) -> List[str]:
        ...
        try:
            ...
            self.file_logger.info(f"Found {len(pdf_files)} PDF files in {self.pdf_folder}")
            return pdf_files
    ...
    def _is_content_valid(self, content_data: PDFContentData) -> bool:
        ...
        if len(content_data.content) < MIN_CONTENT_LENGTH:
            self.content_logger.warning(f"Content too short for {content_data.filename}: {len(content_data.content)} chars")
            return False
    ...
    def process_pdfs(self) -> Dict[str, any]:
        ...
        # Log final results
        self.content_logger.info(f"PDF processing completed - Success: {results['successful']}, Failed: {results['failed']}")
